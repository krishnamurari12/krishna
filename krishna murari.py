...
Original file is located at
    https://colab.research.google.com/drive/198fmJLULflpLRuAh6kp9EijIAU2CKcDD
...
n = 0 # order
x = 0.0
 # besides function of first kind
print ("J_%d(%f = %f)" % (n, x, jn(n,x)))

x =1,0

 # besides function of second  kind
...
fig,ax = plt.subplots()
for n in range(4):
  ax,pl.plot(x, jn(n, x), lebel=r"$J_%d(x)$" %n)
  ax.legend()
...
#
x_lower = 0*(-np,pi); x_upper = np.pi; #limits
val, abserr = quad (f, x_lower, x_upper)

print("integral values =", val, ", absolute error =", abserr)

#.................
# fill a curve with colour
#
...
plt.plot(X, Y, label='f(x) = x^2', color='g')
...
val, abserr = dblquad(integrand, x_lower,x_upper,lambda x : y, lambda x: y_upper)
...
radius = 0.1  # Radius of the sphere (m)
mass = 1.0  # Mass of the sphere (kg)
...
while time <= total_time:
    # Append current values to arrays
    time_values.append(time)
    position_values.append(height)
    velocity_values.append(velocity)

    # Calculate net force (gravity - drag)
    gravity_force = mass * g
    drag_force = 0.5 * air_density * np.pi * radius**2 * drag_coefficient * velocity**2
...
plt.xlabel("Time (s)")
plt.ylabel("Height (m)")
plt.title("Sphere Falling Under Gravity with Air Resistance")
plt.grid(True)
plt.show()
...
position_values = np.zeros(num_time_steps)
velocity_values = np.zeros(num_time_steps)

# Initial values
position_values[0] = initial_height
velocity_values[0] = initial_velocity
...
for step in range(1, num_time_steps):
    ...
    # Update velocity and position using finite difference method
    velocity_values[step] = velocity_values[step - 1] + acceleration * dt
    position_values[step] = position_values[step - 1] + velocity_values[step] * dt
...
plt.xlabel("Time (s)")
plt.ylabel("Height (m)")
plt.title("Sphere Falling Under Gravity (No Air Resistance) - Matrix Format")
plt.grid(True)
plt.show()
...
# Initial values
position_values[0] = initial_position
velocity_values[0] = initial_velocity

# Simulation loop
for step in range(1, num_time_steps):
    # Calculate acceleration using the equation of motion
    acceleration = (-spring_constant * position_values[step - 1] - damping_coefficient * velocity_values[step - 1]) / mass

    # Update velocity and position using the finite difference method
    velocity_values[step] = velocity_values[step - 1] + acceleration * dt
    position_values[step] = position_values[step - 1] + velocity_values[step] * dt
...
plt.title("Spring-Damped Oscillation")
plt.grid(True)
plt.legend()
plt.show()
